...
function parse(option) {
    ...
    const converters = []
    for (const pattern of Object.keys(option.convertPath)) {
        const replacer = option.convertPath[pattern] || []
        const matcher = new Minimatch(pattern)
        const fromRegexp = new RegExp(String(replacer[0]))
        const toStr = String(replacer[1])
